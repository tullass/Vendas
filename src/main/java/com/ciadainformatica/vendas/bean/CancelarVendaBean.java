...
public class CancelarVendaBean implements Serializable {
	...
	public void listarPorDataETipo() {
		...
		try {
			...
			ListaDeVendasPorTipo = vendaDAO.listarPorData(tipo, dataInicio, dataFim); // lista
																						// de
																						// vendas
																						// obtido
			calculaValorTotal();
			...
			if (venda.getValorTotal().doubleValue() > 0) {
				...
				if (tipoDeVenda.getTipo().toString().equals("Dinheiro")) {
					...
					Messages.addGlobalInfo(
							"Valor em dinheiro foi subtraido do sistema, favor devolver ao cliente sem sacar do sistema!");
				} else {
					Messages.addGlobalInfo(
							"como a Venda não foi inteiramente em dinheiro, deve-se estorna o cartão ou sacar dinheiro do sistema manualmente!");
				}
				...
				if (tipoDeVenda.getTipo().toString().equals("Crediario")) {
					...
					Messages.addGlobalInfo(
							"Se esta venda foi no crediário, favor verificar se as parcelas foram apagadas!");
				}
