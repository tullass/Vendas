...
public class CrediarioBean implements Serializable {
	...
	public Crediario getC() {
		return c;
	}

	public void setC(Crediario c) {
		this.c = c;
	}
	...
	public void listar() {
		cliente = null;
		parcelas = new ArrayList<Crediario>();
		aux01 = parcelas = new ArrayList<Crediario>();
	...
	public void procurarParcelas() {
		...
		try {
			...
			if (existe == true) {
				parcelas = crediarioDAO.pesquisar(codigoo);
		...
		for (int i = parcelasSelecionadas.size() - 1; i >= 0; i--) {
			...
			if (valorEmDinheiro.doubleValue() >= 0) {
				...
				try {
					...
					dinheiroDAO.atualizar(parcelasSelecionadas.get(i).getValor());// atualiza
																					// dinheiro
																					// do
																					// sistema
			...
			} else if (valorEmDinheiro.doubleValue() < 0
					&& (valorEmDinheiro.doubleValue() * -1) < parcelasSelecionadas.get(i).getValor().doubleValue()) {
				...
				try {
					...
					DinheiroDAO dinheiroDAO = new DinheiroDAO();
					dinheiroDAO.atualizar(valorEmDinheiro.add(parcelasSelecionadas.get(i).getValor()));// atualiza
																										// dinheiro
																										// do
																										// sistema
	...
	public void excluirParcelaCD() {
		...
		for (int i = 0; i < parcelasSelecionadas.size(); i++) {
			...
			try {
				...
				// DinheiroDAO dinheiroDAO = new DinheiroDAO();
				// dinheiroDAO.atualizar(parcelasSelecionadas.get(i).getValor());//
				// atualiza dinheiro do sistema
