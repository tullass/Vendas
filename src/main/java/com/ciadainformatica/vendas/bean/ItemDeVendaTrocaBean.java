				...
				if (produtos.size() == 1) {// quando produto é encontrado
					...
					for (int i = 0; i < itensNovaVenda.size(); i++) {
						if (itemNovaVenda.getProduto().getCodigo() == itensNovaVenda.get(i).getProduto().getCodigo()) {
							quantidade = (short) (itemNovaVenda.getQuantidade()
									+ itensNovaVenda.get(i).getQuantidade());
							itemNovaVenda.setQuantidade(quantidade);
							itensNovaVenda.remove(i);
							quantidade = 1;
						}
					}
					itemNovaVenda.setValorParcial(itemNovaVenda.getProduto().getPreco()
							.multiply(new BigDecimal(itemNovaVenda.getQuantidade())));
	...
	public void editarValorDoItem(RowEditEvent event) {
		...
		try {
			...
			if (usuario.getTipo() == 'A' || usuario.getTipo() == 'G') {
				...
				itemNovaVenda.setValorParcial(
						itemNovaVenda.getProduto().getPreco().multiply(new BigDecimal(itemNovaVenda.getQuantidade())));
				...
				for (int i = 0; i < itensNovaVenda.size(); i++) {
					if (itemNovaVenda.getProduto().getCodigo() == itensDaVendaDaTroca.get(i).getProduto().getCodigo()) {
						...
						Messages.addGlobalInfo(
								itemNovaVenda.getProduto().getDescricao() + " preco alterado com sucesso!");
				...
				try {
					...
					for (int i = 0; i < itensSelecionadosParaTroca.size(); i++) {
						...
						produto.setQuantidade(
								(short) (produto.getQuantidade() + itensSelecionadosParaTroca.get(i).getQuantidade()));
					...
					if (valorDaDiferenca.doubleValue() > 0) {
						...
						dinheiroDAO.atualizar(valorDaDiferenca);// atualiza
																// dinheiro do
																// sistema
	...
	public void trocarComDiferencaEmC() {
		...
		if (itensSelecionadosParaTroca.size() > 0) {
			if (valorDaDiferenca.doubleValue() >= 0 && numeroDeParcelas > 0) {
				...
				Messages.addGlobalError(
						"Ouve um erro, os produtos selecionados tem um valor inferior a troca, ou o numero de parcelas está incorreto");
			}
		} else {
			Messages.addGlobalError("Ouve um erro, nenhum item de troca selecionado!");
		}
